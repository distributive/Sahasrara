...
To create a plugin, you need to make a Haskell file in the `Plugins` directory. For now, call it `MyPing.hs`. At the top of this file, put the following boilerplate that defines this as a module and imports some neccessary data structures and functions.
...
```haskell
              ...
              let record'@(PingCount _ c) = case user of
                    [] -> PingCount uid 1
                    (x : _) -> (\(PingCount uid' count) -> PingCount uid' (count+1)) $ entityVal x
              liftSql $ repsert (fromJust keyFromRecordM record') record'
              ...
```
...
Having followed the instructions in [the main README](https://github.com/WarwickTabletop/tablebot/tree/main/README.md#environment-file-setup) to set up an environment variable, we can now run the bot, and run our commands!
