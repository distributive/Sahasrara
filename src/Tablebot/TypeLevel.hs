...
import Tablebot.Plugin.Parser (space, word, quoted, digit)
...
parseCommand :: (PComm (ty -> Message -> DatabaseDiscord ())) =>
    ParsedCommand perm ty -> Parser (Message -> DatabaseDiscord ())
