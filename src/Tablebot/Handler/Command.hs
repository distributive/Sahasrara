...
parseCommands :: [Command b] -> Message -> Text -> SeldaDiscord b ()
parseCommands cs m prefix = case parse (parser cs) "" (messageText m) of
  ...
  where parser :: [Command b] -> Parser (Message -> SeldaDiscord b ())
        parser cs = do
            string (unpack prefix)
            choice (map toErroringParser cs) <?> "No command with that name was found!"
          <|> pure (\_ -> pure ())
        toErroringParser :: Command b -> Parser (Message -> SeldaDiscord b ())
