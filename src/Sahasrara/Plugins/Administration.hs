-- |
-- Module      : Sahasrara.Plugins.Administration
...
module Sahasrara.Plugins.Administration (administrationPlugin) where
...
import Sahasrara.Internal.Cache (getVersionInfo)
import Sahasrara.Internal.Types (CompiledPlugin (compiledName))
import Sahasrara.Utility
import Sahasrara.Utility.Database
...
version = Command "version" noCommand []
  where
    ...
    formatVersions vi = "Sahasrara version " <> (pack $ showVersion $ procVersion vi) <> "\nGit Hash: `" <> (gitHash vi) <> "`"
